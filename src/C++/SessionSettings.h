...
public:
  ...
  SessionSettings( std::istream& stream, bool resolveEnvVars = false ) QF_THROW( ConfigError );
  SessionSettings( const std::string& file, bool resolveEnvVars = false ) QF_THROW( ConfigError );

  /// Check if session setings are present
  const bool has( const SessionID& ) const;

  /// Get a dictionary for a session.
  const Dictionary& get( const SessionID& ) const QF_THROW( ConfigError );
  /// Set a dictionary for a session
  void set( const SessionID&, Dictionary ) QF_THROW( ConfigError );

  /// Get global default settings
  const Dictionary& get() const { return m_defaults; }
  /// Set global default settings
  void set( const Dictionary& defaults ) QF_THROW( ConfigError );

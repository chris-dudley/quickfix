...
QF_THROW( ConfigError, RuntimeError )
{
  Locker l( s_mutex );
...
HttpServer::HttpServer( const SessionSettings& settings ) QF_THROW( ConfigError )
...
void HttpServer::start() QF_THROW( ConfigError, RuntimeError )
{
  m_stop = false;

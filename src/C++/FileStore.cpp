...
int FileStore::getNextSenderMsgSeqNum() const QF_THROW( IOException )
{
  return m_cache.getNextSenderMsgSeqNum();
}

int FileStore::getNextTargetMsgSeqNum() const QF_THROW( IOException )
{
  return m_cache.getNextTargetMsgSeqNum();
}

void FileStore::setNextSenderMsgSeqNum( int value ) QF_THROW( IOException )
{
  m_cache.setNextSenderMsgSeqNum( value );
  setSeqNum();
}

void FileStore::setNextTargetMsgSeqNum( int value ) QF_THROW( IOException )
{
  m_cache.setNextTargetMsgSeqNum( value );
  setSeqNum();
}

void FileStore::incrNextSenderMsgSeqNum() QF_THROW( IOException )
{
  m_cache.incrNextSenderMsgSeqNum();
  setSeqNum();
}

void FileStore::incrNextTargetMsgSeqNum() QF_THROW( IOException )
{
  m_cache.incrNextTargetMsgSeqNum();
  setSeqNum();
}

UtcTimeStamp FileStore::getCreationTime() const QF_THROW( IOException )
{
  return m_cache.getCreationTime();
}

void FileStore::reset() QF_THROW( IOException )
{
  try
  {
    m_cache.reset();
...
void FileStore::refresh() QF_THROW( IOException )
{
  try
  {
    m_cache.reset();

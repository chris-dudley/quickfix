...
void Acceptor::initialize() QF_THROW( ConfigError )
{
...
void Acceptor::start() QF_THROW( ConfigError, RuntimeError )
{
  m_stop = false;
...
void Acceptor::block() QF_THROW( ConfigError, RuntimeError )
{
  m_stop = false;
  onConfigure( m_settings );
  onInitialize( m_settings );
...
bool Acceptor::poll( double timeout ) QF_THROW( ConfigError, RuntimeError )
{
  if( m_firstPoll )
  {
    m_stop = false;

...
public:
  ...
  DataDictionary(std::istream& stream , bool preserveMsgFldsOrder = false) QF_THROW( ConfigError );
  DataDictionary(const std::string& url , bool preserveMsgFldsOrder = false) QF_THROW( ConfigError );
  virtual ~DataDictionary();

  void readFromURL( const std::string& url ) QF_THROW( ConfigError );
  void readFromDocument( const DOMDocumentPtr &pDoc ) QF_THROW( ConfigError );
  void readFromStream( std::istream& stream ) QF_THROW( ConfigError );

  message_order const& getOrderedFields() const;
  message_order const& getHeaderOrderedFields() const QF_THROW( ConfigError );
  message_order const& getTrailerOrderedFields() const QF_THROW( ConfigError );
  message_order const& getMessageOrderedFields(const std::string & msgType) const QF_THROW( ConfigError );
  ...
  static void validate( const Message& message,
                        ...
                        const DataDictionary* const pAppID ) QF_THROW( FIX::Exception );

  void validate( const Message& message ) const QF_THROW( FIX::Exception )
  { validate( message, false ); }
  void validate( const Message& message, bool bodyOnly ) const QF_THROW( FIX::Exception )
...
private:
  ...
  QF_THROW( TagNotDefinedForMessage )
  {
  ...
  QF_THROW( RepeatingGroupCountMismatch )
  {

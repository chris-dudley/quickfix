...
Initiator::Initiator( Application& application,
                      ...
                      LogFactory& logFactory ) QF_THROW( ConfigError )
: m_threadid( 0 ),
  m_application( application ),
  m_messageStoreFactory( messageStoreFactory ),
  m_settings( settings ),
...
void Initiator::initialize() QF_THROW( ConfigError )
{
...
void Initiator::start() QF_THROW( ConfigError, RuntimeError )
{
  m_stop = false;
...
void Initiator::block() QF_THROW( ConfigError, RuntimeError )
{
  m_stop = false;
  onConfigure( m_settings );
  onInitialize( m_settings );
...
bool Initiator::poll( double timeout ) QF_THROW( ConfigError, RuntimeError )
{
  if( m_firstPoll )
  {
    m_stop = false;
